  ...
  return (
    <div className="mx-auto max-w-4xl px-4 md:px-16 pt-16 pb-24">
      ...
      <Reveal delay={0.05} className="mt-6">
        <p className="wall-label text-accent">
          {project.plate} — {L(project.category, lang)}
        </p>
      </Reveal>
      ...
      <Reveal delay={0.14}>
        <p className="text-lg text-muted mt-4 max-w-2xl">{L(project.summary, lang)}</p>
      </Reveal>
      ...
      <Reveal delay={0.2} className="flex flex-wrap gap-x-10 gap-y-2 mt-8 text-sm">
        <span><span className="text-muted">{t.projectDetail.year} — </span>{project.year}</span>
        <span><span className="text-muted">{t.projectDetail.role} — </span>{project.role}</span>
        <span><span className="text-muted">{t.projectDetail.stack} — </span>{project.stack}</span>
      </Reveal>
      ...
      <div className="mt-14 space-y-12 max-w-2xl">
        {project.sections.map((section, i) => (
          <Reveal key={section.id} delay={0.1 + i * 0.04}>
            <h2 className="font-display text-2xl md:text-3xl">{L(section.heading, lang)}</h2>
      ...
      {project.plates && project.plates.length > 0 && (
        <div className="mt-16">
          <Reveal>
            <p className="wall-label">{t.projectDetail.plates}</p>
          </Reveal>
